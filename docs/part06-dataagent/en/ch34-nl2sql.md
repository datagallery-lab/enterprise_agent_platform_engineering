...
## 34.14 Change review for query chains

Change review for NL2SQL should inspect the whole query chain, not the generated SQL alone. A small edit can change the user's final number, explanation, and recovery path: a new semantic-layer alias, a default time window, a tighter executor cost limit, or a different model route. Review should first identify which layer changed: question understanding, Linked Schema, candidate Metric, SQL shape, execution plan, explanation template, or frontend state. Each affected layer needs sample comparison so the team does not attribute every difference to the model.

The main review material is a side-by-side run of old and new chains. The platform should replay common questions, high-risk metrics, historical failures, and permission-boundary samples through both versions. The comparison should include Question Frame, semantic-layer version, generated SQL, execution plan, returned row count, truncation marker, explanation text, and EvidenceRef. A different SQL can be acceptable when Metric, filters, result, and explanation strength are the same. A syntactically valid SQL is not acceptable if it switches metric version, drops a permission filter, or changes the default time range. The review record should explain the source of the difference instead of recording only approval.

Semantic-layer drift needs a separate path. Business synonyms, dimension aliases, default metric definitions, and deprecation states change over time. If those changes are not versioned, replaying an old question may produce different candidate metrics, and reviewers cannot tell whether the model improved, the data changed, or the business definition moved. A safer design records the semantic-layer version for every NL2SQL Run and keeps a compatibility window during release. Historical reports and old Trace records keep their original interpretation, while new questions move gradually to the new version. High-risk metrics should run both versions in parallel until the business owner accepts the difference profile.

Executor changes also require review. Lower cost thresholds may turn previously valid long-window queries into resource-limit failures. Stricter read-only validation may reject safe queries that use complex CTEs. New truncation behavior may leave the report layer without enough rows to explain a fluctuation. These changes may be intentional, but they still affect user-visible behavior. Release records should state executor configuration, SQL dialect, cost-estimation rule, and error-code version, then add affected questions to the regression set. Otherwise users only see that a query used to work and now fails, while the platform has no clear explanation.

Rollback should be scoped by data domain. A model-route issue can often be rolled back by tenant or Agent. A semantic-layer definition issue may affect only one metric domain. An executor defect may affect all NL2SQL tasks. The platform should therefore support rollback by tenant, domain, Metric, View, or capability layer, not a global switch alone. After rollback, difference samples should enter the Chapter 39 evaluation set and Chapter 38 Trace review. Each release then improves both capability and the quality system that guards future releases.

---

## 34.15 Reviewing gaps between SQL artifacts and user explanations

After NL2SQL goes live, SQL artifacts and user explanations can diverge. The SQL result may be correct while the answer explains the trend incorrectly. A SQL filter may miss one dimension while the answer still gives a firm conclusion. SQL may return an empty result while the system explains it as a business zero. SQL may use approximate aggregation while the answer omits the error range. These issues do not always come from SQL generation. They often happen during result interpretation and evidence expression.

Gap review should preserve SQL, execution result, and explanation text together. The platform should record user question, Linked Schema, generated SQL, validation result, execution engine, result sample, chart configuration, explanation text, EvidenceRef, and user feedback. When a user disputes the conclusion, the team can tell whether the issue came from SQL, execution, post-processing, charting, or natural-language explanation. Keeping only SQL cannot explain why the report looked wrong to users. Keeping only the answer text cannot locate whether the underlying query was correct.

Repair should follow gap type. If SQL is correct but the explanation is wrong, improve report templates, explanation rules, and LLM-as-Judge samples. If SQL lacks a field while the explanation is firm, improve pre-execution validation and clarification strategy. If an empty result is explained incorrectly, improve prompts for nulls, missing data, and permission trimming. If approximate aggregation omits uncertainty, improve metric metadata and footnotes. Each gap type has its own repair entry, so it should not be treated as one generic answer-quality problem.

A first version can add explanation-review samples for DataAgent query results. The sample set should cover result shapes that often create business disputes: empty result, outlier, year-over-year and period-over-period change, TopN, permission trimming, approximate aggregation, and multi-metric comparison. Chapter 34 then covers SQL generation, execution, and the business explanation users actually read.

## 34.16 SQL failure replay and explanation repair

NL2SQL failure should not be stored only as bad SQL. Many failures come from question understanding, semantic-layer mapping, permission filters, time windows, metric definitions, or result explanation. If the platform records only generated SQL and database errors, the team cannot tell whether to repair the model, semantic layer, or user clarification path. Failure replay should keep the full chain: original question, Question Frame, semantic-layer candidates, generated SQL, validation result, execution result, explanation text, and user feedback.

Replay samples should distinguish failure types. Syntax errors enter SQL generation samples. Field errors enter semantic-layer mapping samples. Permission errors enter policy samples. Empty results enter clarification or data-availability samples. Explanation errors enter report-layer samples. Each type has a different owner. Sending every failure to model tuning slows diagnosis and hides data or permission issues as model issues.

A first version can produce a review package after each NL2SQL failure. The package contains failure label, evidence reference, suggested repair point, and whether the sample enters regression. When a human corrects SQL, the platform should store the reason along with the corrected SQL. DataAgent then turns failures into reusable samples, while SQL generation, semantic layer, and explanation layer improve separately.

## 34.17 User commitments in the query execution chain

After NL2SQL reaches production, a successful demo is not enough evidence. The platform needs stable fields for query scope, resource ceiling, read-only constraint, result explanation, failure reason, and correction record, and those fields should connect to release records, Trace, evaluation samples, and incident notes. When a production issue appears, teams can follow one set of facts to understand scope, ownership, and repair order instead of stitching together model logs, business logs, and verbal explanations.

This evidence also connects the surrounding chapters. It links to Chapter 33 on the semantic layer, Chapter 35 on Python analysis, and Chapter 38 on Trace: upstream capabilities provide assumptions, downstream capabilities consume the result, and governance capabilities preserve evidence and review decisions. If these materials do not share identifiers and versions, the production system splits apart. Business owners see user complaints, platform owners see system errors, and security or compliance teams see explanations written after the fact. That separation makes it hard to decide whether the issue came from data, model behavior, tool contracts, workflow state, or organizational ownership.

Common production risks include users not knowing query limits, approximate results being written as certain conclusions, and failures showing only database errors. These risks are less visible during demos because demos usually exercise the successful path. Production users bring boundary cases, repeated requests, permission changes, and long-running state. The platform team should turn such failures into release samples. Some samples should block launch, some can be handled by degradation, and some require the business owner to accept the remaining risk with a review date.

The query chain should state its answerable scope to users and make failure recovery part of the product experience. The record can stay compact, but it should include time, version, owner, sample, action, and the next review condition. Without those fields, review remains informal experience. With them, one production issue can become material for later releases, evaluation suites, and training.
...
Liu, X., et al. (2025). A survey of Text-to-SQL in the era of LLMs. *IEEE TKDE*, 37(10), 5735-5754. [https://doi.org/10.1109/TKDE.2025.3592032](https://doi.org/10.1109/TKDE.2025.3592032) Tang, Z., et al. (2025). LLM/Agent-as-Data-Analyst: A survey. arXiv:2509.23988. [https://arxiv.org/abs/2509.23988](https://arxiv.org/abs/2509.23988) Lei, F., et al. (2024). Spider 2.0: Evaluating language models on real-world enterprise text-to-SQL workflows. *ICLR 2025*. arXiv:2411.07763. [https://arxiv.org/abs/2411.07763](https://arxiv.org/abs/2411.07763) Gao, D., et al. (2023). Text-to-SQL empowered by large language models: A benchmark evaluation. *VLDB*. arXiv:2305.03111. Pourreza, M., & Rafiei, D. (2023). DIN-SQL: Decomposed in-context learning of text-to-SQL with self-correction. *NeurIPS*. arXiv:2304.11015. [https://arxiv.org/abs/2304.11015](https://arxiv.org/abs/2304.11015) Talaei, S., et al. (2024). CHESS: Contextual harnessing for efficient SQL synthesis. arXiv:2405.16755. [https://arxiv.org/abs/2405.16755](https://arxiv.org/abs/2405.16755) Li, H., et al. (2024). CodeS: Towards building open-source language models for text-to-SQL. *SIGMOD 2024*. arXiv:2402.16347. [https://arxiv.org/abs/2402.16347](https://arxiv.org/abs/2402.16347)
