...
The semantic layer solves the risk of SQL that is syntactically correct but wrong for the business. It organizes metrics, dimensions, join paths, permissions, versions, and default filters above physical tables into a machine-executable business model. DataAgent can use models to understand user language and choose candidates, but the final definition must come from Metric, Dimension, View, and Glossary objects in the semantic layer. This chapter focuses on DataAgent's consumer side. Chapter 15 discusses metadata and metric construction from the data platform perspective; this chapter explains how DataAgent reads the semantic layer during a Run, disambiguates terms, and carries trusted context into answers and Trace.
...
Versioning addresses the issue of a single metric evolving over time, while scope of applicability addresses which users the metric is valid for. A Run must at least record the `metric_id`, `version`, effective time, View, tenant, and default filters. When users ask, "Which GMV did you just use?", the system should be able to answer directly instead of re-explaining in natural language.
...
## 33.13 Handling metric disputes in semantic-layer operations

Metric disputes continue after the semantic layer goes live. The same phrase, such as active customer, new revenue, or churn rate, may be interpreted differently by sales, finance, operations, and management. Once DataAgent turns these metrics into natural-language answers, the dispute becomes more visible because users see a confident conclusion. The platform should treat dispute handling as part of semantic-layer operations instead of explaining each complaint manually.

Dispute handling should start from samples. For each challenged metric, the platform records user question, metric used, semantic-layer version, SQL, data snapshot, answer text, dispute reason, and final ruling. If the dispute comes from wrong field selection, fix schema linking. If it comes from conflicting metric definitions, fix semantic-layer version and scope. If it comes from data delay, fix freshness prompts. If it comes from business disagreement, a business owner must rule. Each cause has a different repair path, so the team should not classify all disputes as model misunderstanding.

The semantic layer should also preserve historical rulings. If a metric definition changes in June, an answer from May should still be explainable under the old version. When users review old meeting material, the platform should not recalculate history under the current definition and overwrite the past. Semantic-layer operations are working when DataAgent answers can follow business change while keeping old answers explainable.

## 33.14 Layered maintenance of semantic-layer sample pools

Semantic-layer samples need layers, or they quickly become a pile of questions that nobody can operate. The first layer is baseline coverage: core metrics, common dimensions, default time windows, and permission pruning. The second layer is business-dispute samples from user challenges, meeting reviews, and metric rulings. The third layer is incident samples that previously caused wrong answers, wrong SQL, or wrong explanations. The fourth layer is release samples that measure the impact of semantic-layer version changes on key tasks. These layers have different owners, release thresholds, and review cadence. They should not all live as one generic question set.

The sample record should keep intermediate decisions together with the final answer. A useful sample includes original user wording, recognized business terms, candidate Metrics, candidate Views, final selection, filtered fields, generated SQL, execution result, explanation text, and EvidenceRef. When a sample fails, the team can locate the failure in term recognition, schema linking, permission pruning, SQL generation, or explanation. A sample that stores only the expected answer turns regression into a black-box test and slows repair.

Layered samples help control release cost. Core metrics should run on every release. Low-frequency business disputes can be reviewed monthly. Historical incident samples should run when related semantic objects change. Exploratory samples can stay in an observation pool. This prevents semantic-layer releases from being delayed by irrelevant tests while still protecting high-risk behavior. For a first platform version, a small number of high-quality samples is more useful than attempting to cover every wording.

The sample pool also supports collaboration. Business owners use samples to confirm definitions. Data teams use them to detect quality and lineage problems. Platform teams use them to validate Linking and Trace. Evaluation teams use them to judge whether DataAgent behavior remains stable. Samples turn metric-definition debates into reproducible material, reducing the cost of repeated meetings. Long-term semantic-layer stability depends on continuous sample maintenance; tidy metric YAML is only the starting point.

## 33.15 User communication for semantic-layer changes

Semantic-layer changes directly change how users understand data. Metric definitions, default time rules, dimension ownership, retired fields, and tighter permissions can all make the same question produce a different answer. If the platform only updates YAML or metric configuration in the backend, users read the change as Agent instability. Semantic-layer releases need user communication, so business teams know which questions are affected, how historical answers should be explained, and when new answers take effect.

User communication does not mean exposing every technical detail. A better approach is to provide short change notes for frequent metrics and key business domains: changed object, old definition, new definition, affected scope, effective time, historical-result handling, and contact owner. When DataAgent answers an affected question, a footnote can state that the metric definition changed on a given date and offer an expanded explanation. Users can then connect number changes to definition changes instead of repeatedly questioning the model and data.

Communication should also enter product interaction. For low-risk metrics, the system can show a change notice after the answer. For high-risk operating metrics, the system should ask for definition confirmation before generating a formal report. For historical meeting material, the system should preserve the semantic-layer version used at the time instead of overwriting old conclusions with the current definition. If a user asks why the number differs from last month, DataAgent should retrieve the semantic-layer change record instead of inventing a plausible explanation.

A first version can create semantic-layer change notices for core metrics. Notices appear in metric detail pages, DataAgent answer footnotes, and report publication records. Each notice links to sample replay and business-owner ruling. The semantic layer then becomes part of user trust and organizational communication, with backend configuration serving that wider purpose.

## 33.16 User communication for semantic-layer changes

Semantic-layer changes directly affect DataAgent answers. Metric-definition changes, dimension hierarchy changes, field alias updates, permission-rule updates, and lineage fixes can make the same natural-language question produce different SQL or different explanations. If users only see answer changes, they cannot tell whether the platform improved or the definition changed. Semantic-layer changes need user communication along with internal data-team release notes.

Communication should be task oriented. Users do not need the full field diff, but they need to know which questions are affected, how the old and new definitions differ, whether historical reports need regeneration, and which scenarios are in transition. For operating-analysis tasks, the platform can tell users that a metric definition changed in a specific version. For formal reports, the system should require new EvidenceRef binding. For automated approvals, the transition window may pause automatic conclusions.

A first version can generate a semantic-layer change summary: affected metrics, affected dimensions, typical questions, sample replay results, historical artifact handling, and business-owner confirmation. The summary enters DataAgent context and report-page notices. The semantic layer then becomes a business-facing definition management tool, with NL2SQL configuration as one consumer.

## 33.17 Impact estimation for semantic-layer changes

After semantic-layer change reaches production, a successful demo is not enough evidence. The platform needs stable fields for metric dependencies, downstream questions, sample coverage, permission change, release time, and user notice, and those fields should connect to release records, Trace, evaluation samples, and incident notes. When a production issue appears, teams can follow one set of facts to understand scope, ownership, and repair order instead of stitching together model logs, business logs, and verbal explanations.

This evidence also connects the surrounding chapters. It links to Chapter 34 on NL2SQL, Chapter 36 on reports, and Chapter 39 on Eval: upstream capabilities provide assumptions, downstream capabilities consume the result, and governance capabilities preserve evidence and review decisions. If these materials do not share identifiers and versions, the production system splits apart. Business owners see user complaints, platform owners see system errors, and security or compliance teams see explanations written after the fact. That separation makes it hard to decide whether the issue came from data, model behavior, tool contracts, workflow state, or organizational ownership.

Common production risks include old questions continuing to hit after metric changes, new permissions not entering the generation chain, and report explanations diverging from SQL results. These risks are less visible during demos because demos usually exercise the successful path. Production users bring boundary cases, repeated requests, permission changes, and long-running state. The platform team should turn such failures into release samples. Some samples should block launch, some can be handled by degradation, and some require the business owner to accept the remaining risk with a review date.

Semantic-layer changes should begin with impact estimation before sample replay and business communication. The record can stay compact, but it should include time, version, owner, sample, action, and the next review condition. Without those fields, review remains informal experience. With them, one production issue can become material for later releases, evaluation suites, and training.
