...
第一版可以先区分三类结果：authoritative、candidate、status。权威结果可以进入报告事实段，候选结果需要进一步选择或复核，状态结果只能驱动任务流转。这个分级很小，但能减少工具调用后的误解释，尤其是候选结果被写成确定结论的问题。
...
第一版可以为每个工具增加生命周期状态：候选、可用、限制使用、冻结、退役。状态变化都要进入 Trace 和评测样本。这样工具目录不会越用越乱，Planner 也不会在旧接口和新接口之间摇摆。Tool Registry 的价值不只在于让模型会调用工具，也在于让工具生命周期能被平台治理。
...
用户承诺应与工具状态一致。若系统告诉用户“已提交审批”，平台必须能证明审批记录已经创建；若只完成了草稿保存，文案就不能写成已提交。前端、报告层和 Trace 都应引用同一份工具结果状态。第一版可以把工具结果分为成功、部分成功、可重试失败、不可重试失败、待人工确认和补偿中。这个状态集虽然简单，但能让工具调用从黑盒动作变成可治理的业务过程。
...
第一版可以选择少量代表场景先建立这种习惯。实践上，应先把高频、高风险、外部可见的路径做扎实，再把样本、台账和复盘方式复制到其他章节对应的能力中。这样扩写后的章节会更接近工程书：它说明能力的定位，也说明能力怎样被接入、验证、运营和退出。
