...
多 Agent 协作、Handoff、Agent Catalog、角色分工、路由、冲突仲裁

## 学习目标

- 能判断一个任务是否需要拆成多 Agent，并说明拆分成本。
- 能设计 Workflow、Question、Data、Report 和 Reviewer 等角色的边界。
- 能说明 Handoff 为什么必须在同一 `run_id` 内完成，而不是让子 Agent 各自启动独立任务。
- 能为并行协作设计冲突检测、权威源选择和人工仲裁策略。
