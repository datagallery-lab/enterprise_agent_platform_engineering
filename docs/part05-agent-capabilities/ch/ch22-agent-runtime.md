...
Agent Runtime、Run 状态机、Step、Tool Call、SSE、检查点

## 学习目标

- 能区分 Run、Step 与 Tool Call 的职责，并说明它们为什么不能混用。
- 能解释 Run 六态的状态迁移，以及终态为什么必须由 Runtime 判定。
- 能设计 Agent SSE 事件，使工具调用、审批和最终结果都可回放。
- 能说明检查点需要保存哪些上下文，才能支持中断恢复和人工接管。
