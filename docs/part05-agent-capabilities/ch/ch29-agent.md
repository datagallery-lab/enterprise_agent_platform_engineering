...
协议互通要先分清对象。MCP 面向模型与工具，A2A 面向 Agent 之间的任务委托，Agent Card 面向能力发现，ACP 面向持续消息协作。企业采用这些协议时，不能让协议适配层绕过内部 Registry、Policy、Run 六态和审计。本章梳理协议版图、各协议的适用边界，以及 mini-platform 中协议适配层的实现约束。

## 关键词

Agent 协议、MCP、A2A、Agent Card、ACP、协议适配层、Registry

## 学习目标

- 能区分 MCP、A2A、Agent Card 和 ACP 的适用对象。
- 能说明外部协议为什么不能替代平台 Runtime 和 Tool Registry。
- 能设计协议适配层，使外部能力仍然进入 `action`、`invoke`、`result` 调用链。
- 能识别 Agent Card、A2A 长任务、MCP tool 版本漂移等生产风险。
