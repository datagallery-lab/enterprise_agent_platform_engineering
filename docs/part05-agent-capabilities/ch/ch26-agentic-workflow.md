...
Agentic Workflow、Reflexion、Self-Refine、Tree of Thoughts、成本计量、按需开关

## 学习目标

- 能区分 Agentic Workflow 增强机制与 Planner 编排模式的分工。
- 能说明 Reflexion、Self-Refine、ToT 各自适合的任务类型与代价。
- 能为增强机制设计开关与计量，避免默认全开拖高成本和延迟。
- 能评估一项增强机制带来的质量提升是否抵得上额外的 token 与时延。
