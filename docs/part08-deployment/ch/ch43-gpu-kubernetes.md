...
GPU 昂贵且稀缺。多个模型、多个租户抢同一批卡时，调度不当会同时出现排队和闲置。GPU 资源池、队列、模型副本、节点隔离和弹性调度共同决定推理服务能否稳定运行。Kubernetes 上的关键设计，是把 GPU 组织成可调度的资源池，用队列和优先级保证关键推理服务拿到资源，并在节点隔离与弹性伸缩之间平衡利用率和稳定性。

## 关键词

GPU 调度、Kubernetes、资源池、队列优先级、节点隔离、弹性伸缩

## 学习目标

- 能说明 GPU 在 Kubernetes 中如何被组织为可调度、可隔离的资源池。
- 能用队列与优先级保证关键推理服务在资源争抢时拿到 GPU。
- 能设计节点隔离策略，避免多租户负载相互干扰。
- 能在利用率与稳定性之间设计 GPU 的弹性伸缩策略。
