...
GitOps、IaC、声明式配置、环境漂移、Terraform、边缘推理

## 学习目标

- 能说明声明式配置相对手工部署解决了什么核心问题。
- 能设计把模型服务与 Agent 平台配置纳入版本管理的 GitOps 流程。
- 能用 Terraform 等 IaC 工具管理基础设施，使环境可重现、可审计。
- 能说明边缘推理在 GitOps 框架下的部署与漂移控制策略。
