...
This chapter examines the model deployment pipeline, covering the engineering requirements for images, weights, service configuration, rolling upgrades, canary releases, rollbacks, and multi-environment promotion. The model serving layer must remain independent of business Agents because model upgrades, weight swaps, and canary rollbacks follow their own cadence and cannot be coupled with application releases. The chapter walks through the complete pipeline from image and weight packaging to multi-environment promotion, compares the applicable scenarios of several deployment frameworks, and explains how rolling upgrades, canary releases, and rollbacks ensure that model updates do not interrupt running tasks.
## Key Terms

Model Deployment, Model Service Layer, Rolling Upgrade, Canary Release, Rollback, Multi-Environment Release
## Learning Objectives

- Be able to explain why the model service layer must be deployed independently from the business Agent.
- Be able to describe the complete pipeline from model container image and weight packaging to multi-environment deployment.
- Be able to compare several model deployment frameworks and select one based on specific application scenarios.
- Be able to design rolling upgrades, canary releases, and rollback strategies to update models without interrupting running tasks.

---
## Opening Scenario

A successful local demo is only the starting point of deployment. Once you move into a shared environment, the question of why an Agent platform needs a dedicated model-serving layer, the choice of deployment framework, and the engineering implementation all simultaneously bear on resource isolation, service stability, release cadence, and failure recovery.
...
### 44.1.2 Misjudgment Risks in Model Serving

**Misconception 1: Docker Compose is enough; no need for KServe**
...
**Misconception 2: A/B testing means running two Deployments with 50% traffic each**
...
**Misconception 3: Embedding services can share the same vLLM instance as LLM**
...
Figure 44-3 highlights the independence between these controls: **Pull complete ≠ Load complete ≠ Accept traffic.** In one deployment, readiness probe was configured at `/health`, causing pods to enter service endpoints during OSS disconnect retries; DataAgent batch requests hit half-loaded pods, increasing error rates though GPU metrics showed normal utilization—because failure happened during loading before inference began.
...
**Trade-off 1: KServe Native Canary vs Service Mesh (Istio)**
...
**Trade-off 2: Model Weight Baked-In Image vs Pull On Startup**
...
| Engine  | Containerization Highlights     | GPU Declaration       | Typical env                      |
|---------|--------------------------------|----------------------|---------------------------------|
...
Another often overlooked aspect is rollback drills. Rolling back a model service is not just a git revert because old model weights, images, secrets, and revisions must still be accessible. If object storage lifecycle policies have deleted old weights or ArgoCD retains only the current tag, rollback becomes meaningless during incidents. Therefore, quarterly drills should test not only KServe patches but also the availability of old weights, readiness of old revisions, and gateway’s ability to rediscover old backends.
...
### 44.3.1 Release Gates and Failure Recovery

**Failure Mode 1: Readiness Probe Too Short Causing “Half-Ready” Pod Serving Traffic**
...
**Failure Mode 2: Canary 5% Traffic Passes but Full Traffic Causes OOM**
...
**Failure Mode 3: Embedding and LLM Share OSS Bucket Prefix Without Versioned Directory**
...
Part VIII’s link closes here in this chapter’s first half: Chapter 43 on quota supply, Chapter 44 on model API supply, and Chapter 45 on unified ingress supply. The deliverable of this chapter is a model service that is deployable, verifiable, and rollback-able—not just a demo “curlable once.”
## Further Reading

- [Chapter 6: Inference Engine](../../part02-model-inference/ch/ch06.md)
- [Chapter 7: Inference Optimization](../../part02-model-inference/ch/ch07.md)
