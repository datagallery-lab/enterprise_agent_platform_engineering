...
*Figure 44-2: Deployment framework selection by workload type, instead of forcing one framework on every job. Source: Self-drawn by this book. Alt text: Online inference, batch inference, and multi-model coexistence workloads respectively connect to more suitable frameworks (KServe, BentoML, Triton), reflecting workload-based selection instead of one-size-fits-all.*

Reading Figure 44-2: the horizontal comparison shows "capability boundaries," not benchmark rankings. The primary LLM path uses KServe because Chapters 44 and 46 require Canary releases, Revision retention, and GitOps declarative publishing all implemented within the same CRD; Embeddings use Triton because their workload characteristics differ from LLMs, and forcing a single framework increases operational complexity. Our recommended selection: **LLM online inference via KServe + vLLM/SGLang Runtime**; **Embedding/Rerank via Triton**; **DataAgent batch analysis keeps Ray Serve as optional path**. Chapter 45's LiteLLM gateway `api_base` uniformly points to the KServe service name, avoiding framework-specific ports.
...
`llm-general-32b` main model uses Canary; `embed-bge-m3` uses parallel two-version runs - new and old each at 50% for 24 hours; after validating metrics with index rebuild job, traffic switches to 100%. Finance forbids revisions entering Canary without passing staged offline evaluation (Chapter 39). Release state machine:
...
Rollback must be a **one-click operation at Revision level**: KServe keeps the previous Stable Revision for 7 days, setting `canaryTrafficPercent: 0` and pinning the old `storageUri`/image digest, avoiding new Helm installs. In one Canary 20% stage, TTFT SLO alarms triggered platform rollback within minutes because ArgoCD Application (Chapter 46) and InferenceService revision history audit fields were aligned. At release review meetings, architects often ask three questions - **Who approves, offline gating evidence, rollback duration** - the state machine structures these answers to prevent "we say rollback works but can't find old Revision onsite."
...
## 44.4 Runtime Evidence After Model Service Launch
...
Rollback plans need to account for state and caches. Online LLM services often switch back to the previous Revision quickly. Embedding, rerank, and batch inference may involve indexes, queues, and intermediate artifacts. A rollback plan should state which services can roll back immediately, which require data rebuild, and which tasks need rerun. Without that plan, teams overestimate rollback speed during incidents. Model serving also needs security hooks. Image scanning, weight-source verification, access logs, network policy, and supply-chain signatures are part of release evidence. Model weights are production dependencies. They should not skip supply-chain checks simply because they are not source code.
...
Model services must record runtime parameters. Temperature, max tokens, context length, batching, quantization format, and tensor parallel settings affect quality and performance. Identical weights with different parameters can behave differently. Release records should include key parameters alongside model names. Batch inference services also need separate queues and retry strategies. Evaluation, offline reports, and index builds should not consume online inference resources. When they fail, they can be rerun as jobs instead of returned immediately like online requests.
...
## 44.5 Model Service Catalog Review

The model service catalog is the operating ledger for the deployment layer. It should record more than model names: Revision, weight URI, image digest, runtime parameters, tensor-parallel settings, context length, gateway route, tenant usage, error rate, P95/P99, GPU hours, rollback state, and service owner. Without this catalog, incident response repeats the same questions: which version handled this request, whether the old version can still take traffic, which tenants still depend on the service, whether the weight came from an approved source, and whether GPU cost can be attributed. Catalog review answers these questions before an incident.

The review should classify services into three states: active, retained for recovery, and ready for cleanup. Active services need SLOs, tenant lists, release records, and incident owners. Services retained for recovery should state recovery time, retention period, and activation conditions. Services ready for cleanup should complete dependency confirmation, traffic drain, weight archival, route removal, monitoring retirement, and GPU release. High-risk models should also record allowed task types, such as internal summarization only, no external customer response, or report generation only after human approval. A catalog entry that says only "general model" invites gateways and applications to use the service outside its verified boundary.

The catalog should connect to evidence systems. A canary release, rollback, weight replacement, or parameter change should have a catalog record that links to Trace, Eval, gateway logs, and the change request. Problems found during review should feed the next deployment improvement cycle. A service without an owner should not receive more capacity. A service without rollback evidence should not enter high-risk workflows. A service without task boundaries should not be opened to more tenants. This shifts model serving from running models successfully once to explaining how models operate over time.

## 44.6 Capacity calibration and service retirement review

After a model service has been in production for a while, its capacity plan needs recalibration. The concurrency, context length, batch settings, and replica count used at first launch usually come from stress tests and early business estimates. Production traffic shows a different shape. Customer-service Q&A may have short input and long output. DataAgent tasks may have long context and short output. Evaluation jobs can occupy GPUs in batches. Report generation may peak on weekday afternoons. If the platform keeps managing capacity by average QPS, it misses KV Cache pressure, streaming connection duration, and batch-job interference.

Capacity review should connect model-serving records with business events. Higher GPU utilization may come from real adoption, retry storms, evaluation tasks entering an online pool, longer context requests, or a tenant bypassing cache. The platform should break down signals by service name, model version, tenant, task type, input tokens, output tokens, queue time, TTFT, TPOT, and fallback state. With that view, the team can decide whether to add capacity, apply rate limits, split a service, or adjust gateway routing. Looking only at node utilization often turns a policy problem into a hardware purchasing request.

Model retirement also needs review. Before an old model is removed, the platform should confirm that gateway traffic is zero, business Agents no longer reference it, evaluation samples have moved, historical Trace can still be explained, the rollback window has expired, and weight plus image retention follows policy. If the model served high-risk workflows, the release evidence and task boundary from that period should remain available. Retirement ends a production commitment. It is larger than deleting a service instance. Weak retirement review leaves two risks: an unknown path still depends on the old model, or incident reviewers cannot recover historical serving evidence.

A first platform version can run capacity calibration and retirement review monthly. Each month, review the services with highest GPU hours, highest error rate, clearest underutilization, and weakest ownership. High-usage services should state whether they need a separate pool, warm-up policy, or stricter limits. Low-usage services should state whether they will be retired, shifted to on-demand startup, or retained for recovery. High-error services should enter incident samples and rollback drills. This cadence keeps the service catalog from growing without cleanup and lets Chapter 43 GPU scheduling, Chapter 45 gateway routing, and Chapter 41 cost governance share the same operating facts.

## 44.7 Degraded routing for model-serving failures

After model services go live, the platform needs degraded routes for failures. Replica crashes, GPU memory fragmentation, slow cold starts, model-image rollback, dependency conflicts, and queue buildup can all make one serving endpoint unavailable. If the upper Agent layer only sees model call failed, users receive a generic error and Runtime cannot choose a safer path. The serving layer should expose failure type, retryability, replacement model, quality impact, and expected recovery time.

Degraded routing should follow task risk. Ordinary summarization can move to a lower-cost model with a visible quality note. High-risk reports can move to async generation or human review. Tasks that depend on a specific tool-call format should not switch to an incompatible model. Compliance-related or externally sent outputs should pause automatic generation first. The model-serving layer and LLM Gateway need shared knowledge of model capability, context length, tool support, tenant permission, and safety policy, so degradation does not break upper-layer contracts.

A first version can define a fallback plan for each production model: replacement model, non-replaceable tasks, trigger threshold, user message, rollback condition, and retest samples. During an incident, the platform follows a prevalidated path instead of deciding the route under pressure. Model serving then moves from successful deployment to maintaining business commitments during failure.

## 44.8 Capability declarations in the model service catalog

The model service catalog should record capability declarations, with service address treated as one field among many. The gateway and Runtime need to know context length, tool-call support, JSON output support, streaming, batch mode, embedding, rerank, concurrency limits, cold-start behavior, and degradation model. If the catalog stores only endpoints, callers discover missing capabilities at runtime or route unsuitable tasks to the wrong service.

Capability declarations should bind to release validation. If a service claims JSON output, it needs structured samples. If it claims long-context support, it needs long-document samples. If it claims NL2SQL suitability, it should pass SQL evaluation. If it can replace another model, it should pass replacement-scenario samples. Each catalog capability should trace to validation evidence and should not rely on manual operator entries alone.

A first version can make the model service catalog emit machine-readable capability descriptions. The gateway routes by capability, Runtime decides whether a task is allowed by capability, and evaluation selects samples by capability. The serving layer then becomes part of the platform contract instead of a set of isolated deployments.

## 44.9 Retirement conditions for model services

After model services reaches production, a successful demo is not enough evidence. The platform needs stable fields for call volume, cost, quality samples, dependent business flows, replacement model, support window, and audit retention, and those fields should connect to release records, Trace, evaluation samples, and incident notes. When a production issue appears, teams can follow one set of facts to understand scope, ownership, and repair order instead of stitching together model logs, business logs, and verbal explanations.

This evidence also connects the surrounding chapters. It links to Chapter 41 on cost governance, Chapter 45 on the gateway, and Chapter 53 on platform operations: upstream capabilities provide assumptions, downstream capabilities consume the result, and governance capabilities preserve evidence and review decisions. If these materials do not share identifiers and versions, the production system splits apart. Business owners see user complaints, platform owners see system errors, and security or compliance teams see explanations written after the fact. That separation makes it hard to decide whether the issue came from data, model behavior, tool contracts, workflow state, or organizational ownership.

Common production risks include old models consuming resources without users, replacement models lacking regression samples, and historical artifacts becoming hard to explain after shutdown. These risks are less visible during demos because demos usually exercise the successful path. Production users bring boundary cases, repeated requests, permission changes, and long-running state. The platform team should turn such failures into release samples. Some samples should block launch, some can be handled by degradation, and some require the business owner to accept the remaining risk with a review date.

The model service catalog should manage launch and retirement together so low-value services do not occupy platform resources indefinitely. The record can stay compact, but it should include time, version, owner, sample, action, and the next review condition. Without those fields, review remains informal experience. With them, one production issue can become material for later releases, evaluation suites, and training.
...
3. Canary releases require distinguishing between liveness and readiness probes, with separate gating for large model cold starts; rollback should be a one-click operation instead of full redeployment.
...
KServe. (n.d.). [Documentation](https://kserve.github.io/website/latest/). BentoML. (n.d.). [Documentation](https://docs.bentoml.com/). NVIDIA Triton Inference Server. (n.d.). [Documentation](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/). Ray Serve. (n.d.). [Documentation](https://docs.ray.io/en/latest/serve/).
