...
The platform should not require lakehouse writers, metadata systems, and DataAgent to interpret each connector's internal state independently. The ingestion layer exposes unified interface contracts downstream. Contracts translate "what tools do" into "what platform guarantees." Connectors track topics, partitions, offsets, cursors, job IDs, or internal checkpoints; DataAgent needs to know table source, update frequency, primary keys, quality status, and query permissions. Without contracts, downstream guesses data state, impeding fault accountability.
...
Tool introductions must serve architectural tradeoffs. Debezium suits Kafka-centric DB CDC event buses; Airbyte fits open-source connector platforms; Fivetran suits managed ELT reducing connector ops; Flink CDC suits real-time transformation and multi-sink CDC pipelines. Choose tools relative to organizational capabilities. Teams already streaming with Kafka have higher control using Debezium or Flink CDC; teams focused on rapid SaaS onboarding prioritize Airbyte or Fivetran connector coverage; handling sensitive data and complex internal permissions requires careful managed service compliance checks. Tools are not absolutely better or worse but must match pipeline roles and team skills.
...
These failure modes commonly appear during restarts, backfills, schema changes, and traffic spikes instead of initial launches. Platforms must monitor "task success" and "data trustworthiness." For example, sync succeeded but duplicate keys cause overcounts; CDC runs but lagging log positions delay answers; schema auto-admits new fields but semantics are undocumented causing Agent misinterpretation.
...
Figure 10-8 illustrates a practical principle: each ingestion pipeline should retain both append-only changelog and query-optimized current tables. Changelog supports audit, replay, and recovery; current supports DataAgent's latest business queries. Only storing current reduces traceability; only storing changelog pushes query complexity downstream. These tables serve "explain the past" and "query the present" goals respectively instead of duplicating effort.
...
Figure 10-9 presents this chapter's mini-platform minimal loop: input source system type, freshness, keys, and target table info; rule model chooses mode and tool; outputs a DataAgent-readable ingestion contract. `mini-platform/infra/ingestion/pipeline_contract.py`:
...
def plan_ingestion_pipeline(request: dict[str, Any]) -> PipelineDecision:
    ...
    if source_kind is SourceKind.DATABASE and freshness <= 300 and has_primary_key:
        return PipelineDecision(
            ...
            tool="Debezium",
            reason="A core database fact table requires minute-level freshness and has a stable primary key.",
...
## 10.4 Replay and downstream impact review for ingestion changes

Ingestion changes need replay. Adding a connector, switching CDC tools, changing primary-key logic, modifying delete semantics, changing a timestamp field, or rerunning historical partitions can affect the semantic layer, metric tables, vector indexes, and DataAgent samples. If the data team checks only that the target table has rows, and the platform team checks only that the Agent can answer, the gap between those checks may still hide a metric change. Replay should use representative business questions that cover high-frequency metrics, historical lookback, delete events, permission fields, cross-table joins, and backfilled partitions. Before and after the change, the platform should compare row counts, key distribution, update time, delete records, quality checks, core SQL results, and final answers.

Downstream impact should be written into the change record before release. A new source field may appear harmless to existing SQL, but it can change schema inference, semantic-layer candidates, and model field selection. A backfill may repair historical data while changing trend charts in old reports. An API pagination fix may recover missing customer records and also require a previous operating conclusion to be explained again. The ingestion layer should notify metadata, quality, semantic-layer, evaluation, and reporting owners. For high-risk domains, a completed ingestion change should trigger replay of key samples and record the result in Trace or the release log.

Replay results should drive the release action. If differences match the expected repair, the release can continue and freshness metadata should be updated. If differences come from backfill or field correction, affected metrics and reports should be marked. If differences come from unexpected duplication, loss, or permission-field errors, downstream use should pause while the data is repaired. The platform should not leave every ingestion change for users to interpret. DataAgent answers rely on data facts; when those facts change, the system needs to state which conclusions remain usable, which need review, and which should be withdrawn.

A first version can keep this mechanism lightweight. Each core data domain can maintain ten to twenty query samples tied to source tables, metrics, permissions, and expected explanations. After an ingestion change, the platform runs those samples and produces a difference summary. This will not catch every data-quality issue, but it creates a release path that connects ingestion and Agent behavior. As incidents accumulate, the sample set can add delete handling, backfill, schema evolution, and cross-system reconciliation cases.

## 10.5 User-visible boundaries for ingestion delay

Data ingestion delay directly affects the trustworthiness of Agent answers. CDC backlog, upstream API throttling, late batch files, schema-evolution approval, and backfill jobs can all make DataAgent see data that lags behind business reality. Traditional data platforms can show these issues in schedulers and monitors. Agent scenarios also need to translate delay into answer boundaries users can understand.

User-visible boundaries should state data cutoff time, delay reason, affected scope, and available substitute. If the latest order data is delayed, the system can return the previous version and mark the cutoff time. If one region is missing, the system should limit the conclusion scope. If backfill is running, the system should avoid firm trend conclusions. If ingestion failure affects a high-risk report, publication should pause and the owner should be notified. Users do not need Kafka offsets or connector logs, but they need to know whether the current answer can support a decision.

Ingestion delay should enter Trace. Whenever DataAgent answers with delayed data, Trace should record ingestion task, data version, delay window, quality state, and user message. Later review can check whether the system informed users correctly and connect data ingestion issues to data-question disputes. Without this path, business teams may blame the model or semantic layer for differences caused by delayed ingestion.

A first version can define delay levels for core data sources. Minor delay allows synchronous answers with cutoff time. Medium delay moves the task to asynchronous reports or asks for user confirmation. High-risk delay refuses the answer or moves to human handling. The ingestion layer therefore moves data into the platform and also communicates data availability to Agents.

## 10.6 Business communication for ingestion delay

Engineering teams often treat ingestion delay as a pipeline metric, while business users experience it as a trust problem. A ten-minute delay in sales details may only affect a live dashboard. A ten-minute delay in order status may lead an Agent to give the wrong fulfillment recommendation. A ten-minute delay in permission labels can create an access risk. The platform needs to translate delay from data-pipeline state into whether the current answer can be used.

Delay communication should happen before the Agent answers. If a source has not finished synchronization, the context package should carry data freshness, last sync time, missing partition, and affected scope. For low-risk queries, the system can show results with the data timestamp. For high-risk recommendations, it should avoid a definitive conclusion or route to human confirmation. Users then know which point in time the answer represents and do not treat stale data as current fact.

The ingestion team should also record delay cause. Source outage, missing incremental field, CDC backlog, permission recomputation failure, and quality-check block require different owners. If all delays appear only as pipeline late, DataAgent cannot decide whether to wait, degrade, or refuse. A first version can maintain delay state, business impact, allowed degradation, and notification target for each source, so ingestion status can support task judgment in upper Agent layers.

## 10.7 Responsibility ledger for data-source onboarding

When responsibility ledger for data-source onboarding reaches production, the platform needs a shared evidence standard for source owner, SLA, primary key, incremental field, masking rule, quality sample, downstream consumer, and stop condition. This standard is not paperwork for its own sake. It lets business, platform, data, security, and operations teams discuss the same facts. Without this material, incident review depends on memory and personal judgment. With it, the team can see which inputs were valid, which actions executed, which artifacts can still be used, and which results need correction or withdrawal.

This evidence should connect to Chapter 11 on ingestion, Chapter 14 on quality governance, and Chapter 33 on the semantic layer. The upstream chapters provide the capability base, downstream chapters consume the runtime result, and this chapter explains how the middle layer is verified. If a capability looks complete inside one chapter but cannot enter Trace, Eval, release records, or the compliance evidence package, the production system still has a break in the chain. Readers should treat cross-chapter interfaces as engineering contracts, not as a reading order.

Common risks include source owners disappearing after launch, field meaning changing with the business system, and downstream Agents continuing to use deprecated fields. A successful demo rarely exposes these problems because demo samples are usually clean, short, and direct. Real business traffic brings stale data, abnormal input, permission changes, user withdrawal, budget limits, and long-running state. If the platform does not turn those situations into samples and ledgers, later scenarios will hit the same class of issues again.

Source owners disappearing after launch should be turned into a tracked review item when it appears repeatedly. The operating record should at least state owner, version, sample, affected scope, action, and review time. It does not need to become a long process report, but it must be clear enough for a later maintainer to understand the decision. For high-risk capability, the record should also state which conditions allow wider use and which failures require degradation or withdrawal.
...
Debezium. (n.d.). [Documentation](https://debezium.io/documentation/). Airbyte. (n.d.). [Documentation](https://docs.airbyte.com/). Apache Flink. (n.d.). [Flink CDC documentation](https://nightlies.apache.org/flink/flink-cdc-docs-stable/). Apache Kafka. (n.d.). [Documentation](https://kafka.apache.org/documentation/).
