...
Data ingestion, CDC, data contract, source system integration, failure recovery, data freshness

## Learning Objectives

- Be able to choose between batch, CDC, file, and API ingestion based on freshness, data volume, and source system type.
- Understand how data contracts constrain source system schema changes to avoid silent downstream damage.
- Design retry, isolation, and alert mechanisms for ingestion failures to prevent dirty data from entering downstream.
- Explain why data quality at the ingestion layer directly determines the trustworthiness of DataAgent’s query results.
...
**Tradeoff 2: Home-built Connectors vs Managed ELT**
...
### 10.2.4 Failure Patterns: Duplication, Disorder, Resume, Schema Drift, and Backfill
...
### 10.3.1 Exposure Gates and Operational Constraints
...
### 10.3.2 Failure Scenarios and Repair Paths

**Failure case 1: Log retention insufficient, Connector recovery fails**

- Symptom: CDC Connector stops for two days over weekend, on Monday cannot find original binlog/WAL position.
- Root cause: Source log retention shorter than downtime; offset points to deleted logs.
- Repair: Increase log retention; monitor stop duration and replication backlog; resnapshot affected tables and reconcile changelog with current.

**Failure case 2: Full snapshot slows source DB**

- Symptom: Initial order detail sync causes noticeable query latency spike in business DB.
- Root cause: Snapshot scanning large tables consumes IO and locks resource.
- Repair: Run snapshot on read-only replica; lower concurrency; shard by primary key range; schedule in low traffic period.

**Failure case 3: Source table has no primary key; target upserts duplicate continuously**

- Symptom: Duplicate business rows appear in lakehouse; DataAgent overcounts orders.
- Root cause: Source has no stable primary key; sink writer cannot identify duplicates.
- Repair: Confirm business unique keys with business; if unavailable, write changelog only, and use downstream window functions to keep latest version.

**Failure case 4: Backfill overwrites real-time increments**

- Symptom: Fixing historical orders causes real-time order status to be overwritten by stale data.
- Root cause: Backfill and CDC sink write to current table simultaneously without version comparison.
- Repair: Write backfills to staging table; merge atomically by source update time and position; pause affected partitions or apply conflict detection during merge.
...
- Related chapters: [Chapter 11 Data Lakes and Lakehouses](ch11.md), [Chapter 13 Stream Computing and Real-Time Data](ch13.md), [Chapter 14 Data Orchestration and Quality](ch14.md), [Chapter 15 Metadata, Lineage, Contracts, and Metrics](ch15.md), [Chapter 34 NL2SQL Engineering](../../part06-dataagent/ch/ch34-nl2sql.md)
