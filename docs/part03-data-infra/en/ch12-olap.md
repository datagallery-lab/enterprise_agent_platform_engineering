...
OLAP engine, multi-engine routing, query governance, Trino, ClickHouse, resource isolation

## Learning Objectives

- Be able to route queries among engines like Trino, ClickHouse, and StarRocks based on query type.
- Be able to apply resource limits, timeouts, and concurrency controls to DataAgent-generated queries.
- Understand how engine-level security boundaries prevent unauthorized access and runaway queries.
- Identify common factors affecting OLAP query performance and approaches to optimization.
...
Routers cannot return just an engine name. They must also indicate workload type, SQL dialects used, table and column access rights, scan budgets, and fallback permissions on failure. For DataAgent, dialect differences are critical: the same date function, JSON function, or approximate aggregation may differ in syntax and semantics across engines. Platforms must either fix dialect before SQL generation or perform syntax transformation and validation before submission.
...
Step one is to separate queries into "recurring" and "ad hoc." Recurring queries should push cost forward via precomputation, materialized views, caches, and wide tables; ad hoc queries require controlled scanning and concurrency to avoid a few exploratory requests overwhelming the cluster. This is critical for DataAgent, which can automatically launch multiple exploratory queries in a conversation—without budgets or cache, a simple follow-up can become an expensive query batch.
...
**Tradeoff 1: Single Engine Unification vs Multi-Engine Collaboration**
...
**Tradeoff 2: Federated Query vs Pre-Built Datasets**
...
### 12.3.1 Release Gates and Runtime Constraints
...
### 12.3.2 Failure Scenarios and Fixes

**Failure mode 1: Using Trino as a high-concurrency report database**

- Symptom: DataAgent and BI dashboards share Trino; ad hoc cross-source queries slow fixed operational dashboards.
- Root cause: Trino suits open lakehouse gateway and cross-source exploration but lacks hotspot materialization and report isolation.
- Fix: Migrate fixed dashboards to StarRocks, Doris, ClickHouse, or Snowflake warehouse; keep Trino for exploration with concurrency and scan limits.

**Failure mode 2: Only consider engine performance, ignore data layout**

- Symptom: Load tests perform well; post-launch some queries remain slow.
- Root cause: Sort keys, partitions, materialized views, stats, and skew not designed for real queries.
- Fix: Gather top query patterns; redesign primary keys, sort keys, partitions, materialized views, hot-cold tiers; run regression tests with real distribution.

**Failure mode 3: Inconsistent permissions across engines**

- Symptom: Same user cannot see a column in BI but can query it in another SQL client.
- Root cause: Platform metadata, engine internal permissions, and external catalogs lack unified publishing flow.
- Fix: Platform policies take precedence; auto-generate engine permissions; new engines first connect catalog adapter and policy controller, then execution adapter.

**Failure mode 4: Local DuckDB analysis irreproducible in production**

- Symptom: Data scientists get conclusions reading files in Notebooks, but production dashboards differ.
- Root cause: Local analysis bypasses semantic layers, data versioning, and metric definitions.
- Fix: Use DuckDB for exploration only; record input files, lakehouse snapshots, SQL, and metric definitions; convert to governed datasets or metric layer queries for production.
...
- Related chapters: [Chapter 10 Data Ingestion and Integration](ch10.md), [Chapter 11 Data Lake and Lakehouse](ch11.md), [Chapter 15 Metadata, Lineage, Contracts, and Metrics](ch15.md), [Chapter 34 NL2SQL Engineering](../../part06-dataagent/ch/ch34-nl2sql.md)
