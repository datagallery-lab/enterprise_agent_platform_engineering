...
Stream processing, event time, window, state management, exactly-once, real-time features

## Learning Objectives

- Be able to determine whether a scenario truly requires real-time processing and the costs involved.
- Distinguish event time from processing time and select appropriate windows and watermarks for out-of-order data.
- Explain how stream state management and exactly-once semantics ensure no duplicates or omissions in results.
- Design a pipeline to stably supply real-time features to Agent online analytics.
...
- DataAgent can query not only historical tables but also real-time metrics and event context.
- Business Agents can not only answer “what happened” but also trigger alerts, dispatch, freeze, downgrade, restock, and other actions.
...
- Data governance oversees not only tables and fields but also event contracts, delay SLAs, replay windows, and anomaly correction processes.
...
| Backpressure | When downstream cannot keep up, upstream processing slows down | Signals capacity bottlenecks; not just “job slowdown” |
...
**Trade-off 1: Batch, Micro-batch, and Continuous Stream**
...
**Trade-off 2: Flink, Spark Structured Streaming, Kafka Streams**
...
**Trade-off 3: Speed Priority vs Completeness Priority**
...
Real-time metric service responses cannot just return a single number. If DataAgent only sees `"0.982"`, it cannot tell if the value is from a complete window, includes late event corrections, or is in replay.
...
**Example 13-1: Real-Time Metric Service Response Example.** Production engineering example where `is_final=false` tells DataAgent that the current window may be corrected by late events; query responses should indicate “as of current watermark.”
...
**Example 13-2: Real-Time Event Contract Example.** Focuses on encapsulating business time, production time, schema version, partition key, quality flags, and governance metadata in a unified envelope.
...
**Trade-off 4: Where to Write Real-Time Results**
...
**Trade-off 5: Agent Reads Stream Directly or Through Service Layer**
...
**Example 13-3: Real-Time Context Service Response Example.** This interface lets Agents explain “data as of which watermark,” “whether results are final,” and “allowed triggered actions.”
...
Figure 13-10 shows that stream jobs should not be deployed direct from dev to production. Pre-release replay and shadow comparisons are key to detecting definition deviations; Savepoints are critical control points for stateful job upgrades and rollbacks; after canary rollout, monitoring latency, output quality, and checkpoint metrics is essential, not just process health.
...
**Example 13-4: Real-Time Job Configuration Example.** Key is explicitly defining event time, watermark, state retention, idempotent keys, and governance strategy rather than just framework parameters.
...
**Example 13-5: Window Metric Pseudocode.** This expresses the calculation intent to generate 5-minute event-time windows with a primary key ensuring downstream idempotent update.
...
|---|---|---|
| Release | Version numbering, config snapshots, Savepoint, rollback strategy | Stateful stream job upgrades require state compatibility checks, not just image replacement |
...
- [ ] Business value: Confirm real-time pipeline serves low-latency actions like alerts, blocking, scheduling, and online context—not just for “looking real-time.”
- [ ] Event contract: Each event has event_id, event_time, schema_version, partition_key, owner, retention, and PII tags.
- [ ] Time semantics: Declare use of event time, ingestion time, or processing time; critical metrics specify watermark and allowed lateness.
...
**Failure Mode 1: Using Processing Time for Payment Success Rate Causes False Alarms at Promotion Peak**

- Symptom: After promotion starts, some stores’ 5-minute payment success rate suddenly drops; risk control Agent auto downgrades payment channel.
- Root cause: Events windowed by processing time. Peak backlog causes early payment events processed late, counted in later windows, suppressing current window success rate.
- Fix: Change payment metric to event-time windows with 2-minute out-of-order tolerance and 10-minute late update allowance; DataAgent responses display watermark and finality flags.

**Failure Mode 2: Kafka Partition Key Using City Causes Core Cities’ Partitions to Hot-Spot**

- Symptom: Most stores’ metrics normal nationwide; Shanghai and Shenzhen stores experience persistent delay.
- Root cause: Partition key is `city_id`, concentrating traffic in few partitions for big cities, causing single task bottlenecks.
- Fix: Use `store_id` as partition key; for hotspot stores, secondary partition splits; window aggregates perform local partial aggregation before global merge.

**Failure Mode 3: Deduplication State Without Expiration Causes Increasing Recovery Time**

- Symptom: After several weeks, checkpoint duration grows from tens of seconds to tens of minutes; failure recovery exceeds acceptable business time.
- Root cause: Deduplication state stores all historical event IDs without cleanup by event time or business cycle.
- Fix: Set state expiration based on event retention; event IDs beyond late window move offline to reconciliation, no longer occupy stream state.

**Failure Mode 4: Non-Idempotent Alert Sink Leads to Duplicate Tickets After Recovery**

- Symptom: After cluster restart, multiple payment anomaly tickets for same store created.
- Root cause: Stream job recovers from checkpoint and replays partial results; alert system only appends, no business key dedup.
- Fix: Define `alert_id = rule_id + store_id + window_start`; alert system upserts by `alert_id`; recovery merely updates existing alert state.

**Failure Mode 5: Retaining Only Aggregate Results Unable to Explain DataAgent Answers**

- Symptom: Business questions “Why is this store abnormal in last 10 minutes?” but DataAgent only returns aggregate value, no supporting events listed.
- Root cause: Raw event retention too short; cleaned details not written to lakehouse; real-time serving layer is only source of truth.
- Fix: Always write key events to lakehouse details; retain lineage fields for real-time aggregates; DataAgent answers attach window info, event counts, and sample event references.
