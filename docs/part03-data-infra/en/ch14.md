...
Data orchestration, DAG scheduling, quality gate, data product, task dependency, scheduling recovery
## Learning Objectives

- Be able to describe task dependencies using DAGs and design failure retry and backfill strategies.
- Be able to set quality gates at key points in data pipelines to block abnormal data from flowing downstream.
- Be able to explain the differences between data products and scattered scripts in terms of contracts, versioning, and subscribability.
- Be able to evaluate and select orchestration tools like Airflow and Dagster based on team scenarios.
...
| Quality Gate | Rules-checking, blocking, degrading, or alerting before data enters downstream consumption | Focuses on trustworthiness; not just monitoring dashboards |
...
**Example 14-1: Quality Event Contract Example.** This is a production engineering example. It allows DataAgent and dashboards to know whether a partition is available, why it was blocked, and what temporary fallback strategy to use.
...
| Consistency | Are cross-table or cross-system values aligned? | Amount differences between order fact and payment fact tables within threshold | Pause critical queries and trigger reconciliation |
...
**Tradeoff 1: Hard Gates, Soft Alerts, and Sideband Isolation**
...
**Example 14-2: Asset orchestration and quality configuration example.** This is not proprietary syntax but expresses the key fields production systems need to store: dependencies, freshness, gates, failure actions, fallback strategies, and notification channels.
...
**Example 14-3: Publication gate pseudocode.** The core idea is to write temporary results first, then validate quality, finally switch versions; on failure, retain the old version and expose the quality status.
...
### 14.3.1 Publication Gates and Fault Recovery
...
**Failure Mode 1: Task succeeds but orders fact table is empty; DataAgent returns incorrect conclusions.**

- Symptom: The ops team asks about yesterday’s fulfillment delay; DataAgent answers "no obvious delay," but in fact the upstream order fact table collection failed producing an empty table.
- Root cause: The scheduler only checks task exit status, with no quality gates on row count, partition freshness, or upstream asset state.
- Fix: Core assets add row count fluctuation, partition freshness, and non-empty checks; on quality failure, block publication and make DataAgent return "data unavailable" status.

**Failure Mode 2: Backfilling historical partitions refreshes online cache, causing user-visible metric jumps.**

- Symptom: When the data team backfills last month’s fulfillment criteria, operational dashboards and DataAgent current metrics temporarily spike.
- Root cause: Backfill tasks reuse official publication processes without distinguishing historical shadow partitions and online partitions; no downstream cache refresh notification.
- Fix: Backfill first writes shadow versions; after quality recheck and diff report, switch manually; isolate current window cache from historical backfill cache.

**Failure Mode 3: Promotion day row count spike triggers false quality alarms, core daily reports get blocked.**

- Symptom: On a big sale day, order volume exceeds historical thresholds; quality system flags this as abnormal and blocks publication.
- Root cause: Row count rules only use the past 7-day average, without incorporating business calendar, promotion tags, or weekday effects.
- Fix: Incorporate business calendar into threshold models; use a separate baseline during promotions; keep only uncompromisable rules as hard gates, convert fluctuation rules to soft warnings.

**Failure Mode 4: Quality rules have no owners; alerts persist for weeks without handling.**

- Symptom: An effectiveness alert for a dimension table triggers daily, but no blocking or fixing happens.
- Root cause: Rules have no owner; alerts lack escalation path; quality platform only logs failure counts.
- Fix: Bind each rule to an asset owner; automatically escalate after confirmation timeout; long-term false alarms must be closed, adjusted, or converted to observation.

**Failure Mode 5: Agent user feedback is not integrated into the quality system; repeated similar errors appear.**

- Symptom: Multiple business users report "store region definition differs from dashboards," but DataAgent repeats the same incorrect answers a week later.
- Root cause: User feedback only goes to product support queues, without being consolidated into data quality rules or semantic definition checks.
- Fix: Establish DataAgent feedback queues; aggregate dispute samples by asset and field; after owner review, add consistency rules for region mapping.
...
- Related Chapters: [Chapter 10 Data Ingestion and Integration](ch10.md), [Chapter 11 Data Lakes and Lakehouses](ch11.md), [Chapter 12 Lakehouse Engines and OLAP](ch12-olap.md), [Chapter 13 Streaming Computing and Real-time Data](ch13.md), [Chapter 15 Metadata, Lineage, Contracts, and Metrics](ch15.md), [Chapter 34 NL2SQL Engineering](../../part06-dataagent/ch/ch34-nl2sql.md), [Chapter 38 Observability and Tracing](../../part07-observability-eval/ch/ch38-trace.md), [Chapter 42 SLO and Reliability](../../part07-observability-eval/ch/ch42-slo.md).
