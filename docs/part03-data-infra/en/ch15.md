...
Metadata, data lineage, data contracts, metric definitions, control plane, traceability

## Learning Objectives

- Explain what queryable semantics metadata as a "data control plane" provides to DataAgent.
- Use data lineage to perform upstream impact analysis and root cause localization of data incidents.
- Use data contracts to constrain producers and prevent silent upstream changes from breaking downstream systems.
- Explain why unified metric definitions are a prerequisite for trustworthy DataAgent queries.
...
The data catalog is the entry point of the metadata control plane. It answers “What data exists? Who owns it? Can I use it? For which questions is it suitable?” A catalog fit for DataAgent should not only show table and field names but also asset type, business description, owner, classification, quality status, freshness, usage heat, downstream consumers, and sample queries.
...
*Figure 15-3: Asset profiles serve consumption behavior. Source: Author. Alt text: Asset profile elements (owner, classification, quality, heat, definitions) connect to specific consumption behaviors (trustworthiness, usability, contact person), emphasizing profiles support consumption decisions rather than metadata dumping.*

Figure 15-3 emphasizes that asset profiles must serve consumption behavior. When DataAgent selects tables, it should consider not just field matches but also quality status, freshness, owner, and applicable scenarios. For example, “fulfillment delay” might exist in detail tables, daily summary tables, and real-time wide tables. For “yesterday’s root cause analysis,” daily or detail tables suit better; for “real-time anomaly,” the wide table fits.
...
Lineage describes the source of data, what processing it underwent, and which downstream objects it affects. For the Agent platform, lineage is used not only for engineering troubleshooting but also for answering citation, impact analysis, and compliance audit. If DataAgent answers “The rise in East China delays mainly comes from night warehouse distribution,” the platform should reveal which assets, partitions, definitions, and quality states were used.
...
A Data Contract is a formal agreement between producers and consumers on data assets. It constraints not only schema but also business semantics, refresh SLA, quality rules, permission categories, compatibility policies, and change processes.
...
**Example 15-1: Data Contract Example.** This contract enables the platform to automatically detect if field, semantic, SLA breaches, or permission changes affect DataAgent.
...
DataAgent’s use of metadata involves four key aspects.

First, **Schema Linking:** the platform must map natural language terms like “East China,” “fulfillment delay,” “yesterday,” “store” to candidate assets, fields, dimensions, and metrics. This mapping needs field aliases, business tags, sample questions, usage heat, and permission filters.

Second, **Definition Explanation:** Agent should not only return values but also explain metric definitions, filters, time ranges, numerator/denominator. For example, “fulfillment delay rate” should clarify if calculated by order count, whether canceled orders are excluded, whether promised time comes from order or fulfillment system.

Third, **Citation Trace:** each answer must record used metrics, asset versions, partitions, query statements, quality status, and permission policies. When users ask “Where does this conclusion come from?,” the platform provides readable citations.

Fourth, **Impact Analysis:** before changing fields, tables, quality rules, or metric definitions, the platform should know what dashboards, Agent capabilities, historical answers, and alert rules will be affected.
...
Figure 15-8 illustrates metadata is not offline documentation but part of Agent runtime. Each tool call should carry identity, permissions, asset version, and quality state; each answer should embed citations and audits.

Example interface request showing how DataAgent queries metadata service for context:
...
**Example 15-2: DataAgent Metadata Context Interface Example.** This production example highlights placing semantic parsing, permission filtering, quality status, and lineage hints together in one response to avoid Agent guessing.
...
| Full-Chain Auditing   | Enables accountability, retrospection, compliance proofs | Increased log storage and retrieval costs | Involving sensitive data, regulated or critical operations | Core Agent capability              |
...
**Example 15-3: Metric Definition Example.** Such definitions allow DataAgent to reuse metric definitions instead of dynamically stitching definitions each query.

Below pseudocode shows DataAgent’s pre-query metadata checks.
...
**Example 15-4: Pre-query Metadata Check Pseudocode.** The core logic: search, authorize, parse metric, check quality and lineage, then record audit.

Change management must also enter the control plane.

![Figure 15-11: Moving Change Validation Before Release](../../images/part3/en/ch15-11.png)
*Figure 15-11: Moving change validation before release. Source: Author. Alt text: flowchart showing schema or definition changes go through contract validation and impact analysis to confirm no downstream issues before release, with arrows indicating prevention rather than firefighting post-deployment.*
...
### 15.3.1 Release Gates and Incident Recovery
...
**Failure Mode 1: Same-name metrics spread across multiple dashboards, causing DataAgent definition inconsistency.**

- Symptom: Business users ask repeatedly about “sales” and “GMV,” and DataAgent uses order amount, payment amount, and net amount after refunds inconsistently.
- Root Cause: Metrics are scattered in dashboard SQL, temporary wide tables, and analysis scripts without unified IDs, definitions, and deprecation.
- Fix: Establish metric registration process; move core metrics into semantic layer; DataAgent calls only active metrics and shows definitions in answers.

**Failure Mode 2: Field type unchanged but business meaning changed, contract check misses this.**

- Symptom: Fulfillment delay rate suddenly drops but warehouse distribution has not improved in reality.
- Root Cause: Upstream changed `delivered_at` from actual receipt time to system confirmation, timestamp type unchanged, schema validation passed.
- Fix: Data contracts include business semantics and incompatible change types; semantic changes require impact analysis and downstream owner approval.

**Failure Mode 3: Lineage stops at table level, unable to judge impact of field changes.**

- Symptom: After store region field adjustment, multiple regional metrics show anomalies but platform only sees table dependencies, unable to pinpoint affected metrics.
- Root Cause: Only job-level and table-level lineage collected, no field-level SQL or metric definition parsing.
- Fix: Add field-level lineage for core assets; metrics explicitly declare dependent fields; change process automatically lists affected metrics and Agent question templates.

**Failure Mode 4: Permission enforced only at database level, Agent answers leak sensitive aggregated dimension details.**

- Symptom: Regional managers disallowed customer detail view get detailed anomalous customer lists via natural language follow-up questions.
- Root Cause: DB permissions block some fields but semantic layer lacks minimum aggregation checks and answer-level masking.
- Fix: Policy service adds metric/dimension/action-level permissions; DataAgent performs aggregation level checks and sensitive masking before output.

**Failure Mode 5: Metadata collection latency causes Agent to use deprecated assets.**

- Symptom: An old fulfillment table has migrated but DataAgent still uses old table for some queries.
- Root Cause: Catalog collection lag causes asset deprecation state not synced to Agent’s search index.
- Fix: Use event push for asset state changes; block queries on deprecated assets; fallback to live catalog queries when index refresh fails.
...
4. Data contracts must cover schema, business semantics, SLA, quality rules, permissions, and change processes—not just field type validation.
...
- Related chapters: [Chapter 10 Data Collection and Integration](ch10.md), [Chapter 11 Data Lake and Lakehouse](ch11.md), [Chapter 12 Lakehouse Engine and OLAP](ch12-olap.md), [Chapter 13 Streaming and Real-time Data](ch13.md), [Chapter 14 Data Orchestration and Quality](ch14.md), [Chapter 32 DataAgent Overall Architecture](../../part06-dataagent/ch/ch32-dataagent.md), [Chapter 34 NL2SQL Engineering](../../part06-dataagent/ch/ch34-nl2sql.md), [Chapter 38 Observability and Tracing](../../part07-observability-eval/ch/ch38-trace.md), [Chapter 51 Security Guardrails](../../part10-security-org/ch/ch51-guardrails.md).
