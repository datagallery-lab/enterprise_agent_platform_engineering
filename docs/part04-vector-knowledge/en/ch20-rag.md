...
RAG, chunking strategy, hybrid retrieval, multi-hop retrieval, citation verification, retrieval quality evaluation
## Learning Objectives

- Be able to design chunking and context assembly strategies based on document structure and question types.
- Be able to combine vector retrieval and keyword retrieval, and use re-ranking to fuse results from both approaches.
- Be able to handle complex multi-evidence questions using query rewriting and multi-hop retrieval.
- Be able to design citation verification and retrieval quality assessment to ensure answers are traceable and reproducible.

---
## Opening Scenario

RAG is not just "vector database + prompt." Enterprise RAG is an evidence production pipeline: converting user questions into retrievable intents, retrieving candidates from multiple indexes, performing permission filtering, ranking and fusion, context assembly, and citation verification before handing the evidence over to the LLM. If any step in this chain fails, the end result will appear as the "model hallucinating."
...
DataAgent’s RAG does not just answer “what the document says”—it also serves NL2SQL and analytic actions. If retrieval results include field explanations, metric definitions, example SQL, data quality rules, and permission constraints, error-prone table, field, and metric mismatches can be reduced before SQL generation. Trusted answers here are not just a natural language snippet but “why the SQL is written this way, which definitions are referenced, and which fields have execution privileges.”
...
The goal of enterprise RAG is to produce answers that are supported by evidence—not merely answers that sound plausible. A trustworthy answer must satisfy at least three requirements: citations are locatable, the answer is consistent with its citations, and both access permissions and temporal validity are confirmed. Evaluation frameworks such as Ragas decompose metrics like context precision, context recall, and faithfulness precisely because "retrieving good material" and "the model faithfully using that material" are two different things.
...
*Figure 20-5: Citation verification UI for trustworthy answers. Source: product UI screenshot. Alt text: The interface annotates each conclusion with a link to the source excerpt; hovering highlights the original passage in context; sentences without supporting evidence are highlighted in red, making every sentence of the answer independently verifiable.*
## Chapter Recap
...
- RAG is not just vector database plus prompt; it is a complete pipeline of parsing, indexing, retrieval, ranking, assembly, generation, and verification.
- Chunking strategies must align with document structure and citation requirements, not just fixed-length splits.
