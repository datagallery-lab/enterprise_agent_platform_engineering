...
This chapter discusses vector databases and indexing algorithms, focusing on platform boundaries, ANN trade-offs, metadata filtering, multi-tenant permissions, and vector library selection. A vector library is not just a "place to store vectors"; it must simultaneously satisfy fast approximate nearest neighbor search, correct metadata filtering, and multi-tenant access isolation—often these requirements conflict. This chapter reviews trade-offs in ANN indexes such as HNSW and IVF, explains why metadata filtering and permissions must be enforced during retrieval rather than after, and provides criteria for selecting mainstream vector libraries.
## Key Terms

Vector database, ANN index, HNSW, metadata filtering, multi-tenant access control, vector database selection
## Learning Objectives

- Be able to explain the trade-offs of ANN indexes such as HNSW and IVF in terms of recall rate, latency, and memory usage.
- Be able to explain why metadata filtering needs to take effect during retrieval rather than filtering after recall.
- Be able to design multi-tenant permission isolation for vector databases to prevent cross-tenant data leakage.
- Be able to choose a vector database for scenarios based on scale, filtering requirements, and operational capability.
...
| Most critical security concern?       | Prevent unauthorized candidates from entering the model, logs, or traces; prioritize pre-filtering for high risk scenarios. |
| Most critical cost concern?            | Dimension, index type, filtering policy, top-k, and re-ranking affect memory and latency; evaluate jointly. |
...
The vector store benchmark report in Figure 18-5 follows the same approach: quality metrics, system metrics, and governance metrics must all be presented on the same page, because enterprise selection is not just about choosing the library with the highest recall, nor just the one with the lowest latency.
