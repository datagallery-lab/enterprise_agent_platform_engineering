...
embedding fine-tuning, reranking, contrastive learning, hard negative samples, regression evaluation, grayscale release
## Learning Objectives

- Be able to determine whether retrieval quality issues should be addressed through embedding fine-tuning, re-ranking, or chunk adjustment.
- Be able to construct positive samples and hard negatives for contrastive learning, and explain why hard negatives are critical.
- Be able to explain the position of the re-ranking model in the recall pipeline and its division of labor with the embedding model.
- Be able to use regression evaluation and canary release to ensure embedding or re-ranking updates do not cause quality regressions.
...
Whether fine-tuning and reranking can be deployed depends on the evaluation feedback loop. A usable enterprise evaluation dataset must cover real queries, golden documents, hard negatives, permission filtering, business scenario labels, and failure reasons. The evaluation report should not only provide an average score but also indicate “which scenarios improved, which worsened, and how cost and latency changed.” The deployment checklist in Table 17-7 should also revolve around these issues.
