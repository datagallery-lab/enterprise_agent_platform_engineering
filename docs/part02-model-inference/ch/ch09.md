...
能力定制、RAG、微调、LoRA、偏好对齐、评测闭环

## 学习目标

- 能根据失败样本判断问题属于知识缺失、格式不稳、任务能力不足还是偏好边界不合适。
- 能说明 Prompt、RAG、微调和对齐各自改变模型链路的哪个部分。
- 能解释 LoRA / QLoRA、全量微调、外部知识库和工具查询之间的工程取舍。
- 能设计一条从线上反馈、样本治理、评测到灰度发布的能力定制流程。
...
如果模型不知道上周刚更新的休假政策，应该更新知识库，不要训练模型记住新政策。如果模型能理解问题，却总是不按固定 JSON 输出，应先检查 Prompt、schema 和结构化输出链路。如果模型长期写出错误 SQL 模式，说明任务分布与基座模型差异较大，才可能需要 SFT 或 LoRA。如果模型在灰区问题上喜欢给出过度承诺，就要处理偏好和安全边界。
