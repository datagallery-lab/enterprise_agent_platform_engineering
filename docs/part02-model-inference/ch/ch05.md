...
模型选型、任务画像、模型路由、模型矩阵、SLO、成本治理、开源与闭源

## 学习目标

- 能从任务画像、数据边界、SLO 和成本四个维度为一个具体任务确定选型约束。
- 能设计一套让多个模型按任务路由的模型矩阵，而非全局押注单一模型。
- 能权衡闭源 API 与开源自部署、大模型与小模型在质量、成本、合规上的取舍。
- 能说明模型选型为什么是可随迭代调整的运行时决策，而非一次性采购。
