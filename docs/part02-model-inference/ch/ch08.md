...
提示工程、结构化输出、JSON Schema、工具调用、约束解码、失败恢复

## 学习目标

- 能把 Prompt 模板写成可版本化的输入契约，而不是散落在代码里的文案。
- 能为模型输出设计小而明确的 schema，并解释字段、枚举、证据和失败出口。
- 能区分 Prompt 约束、约束解码、后置校验和业务校验各自解决的问题。
- 能为工具调用设计参数校验、幂等、权限和人工确认边界。
