...
Online evaluation, LLM-as-Judge, manual sampling inspection, online canary release, continuous optimization, judge bias
## Learning Objectives

- Be able to explain the necessity and coverage differences of online evaluation compared to offline benchmarks.
- Be able to design the LLM-as-Judge scoring process and identify and correct its biases.
- Be able to combine model judging with manual spot checks and conduct risk-stratified sampling for review.
- Be able to feed online evaluation results back into canary releases and continuous optimization closed loops.
...
Hence, feedback records must not only save a `thumb_down` field. They must link user signals with execution evidence:
...
This marks the distinction between Agent-as-Judge and regular LLM-as-Judge. Ordinary text judges can rate the report’s fluency but struggle to know if the agent overlooked a regional order file, confused logistics costs as product master data, or wrote policy suggestions without reading priority rules. Agent-as-Judge judges not only the `final_answer` but also file read logs, tool call results, generated artifacts, dependency graphs, and rubric checkpoints. For enterprise platforms, this design translates directly to BI reports, credit analyses, procurement risk control, and cash flow forecasting: the judge must verify evidence paths behind reports, not just assess whether the report superficially looks professional.
...
Multi-turn Agent interactions complicate experiment attribution. One user session may span multiple runs, and satisfaction at the third turn may depend on the first turn’s answer. If bucketing is done at each request randomly, the same user in one session may experience both group A and B, polluting both experience and data. Therefore, a safer approach is bucketing by user, tenant, or session, and recording the experiment version in context packages. Long tasks also require handling delayed feedback: report generation may take minutes, and downloads or report adoption can happen even later. The experiment window should not only cover the request completion instant.
