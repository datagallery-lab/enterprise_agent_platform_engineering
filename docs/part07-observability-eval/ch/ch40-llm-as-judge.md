...
离线 benchmark 覆盖不了真实流量的长尾。Agent 上线后，团队仍要判断质量是否退化、裁判是否可信、改动是否真的改善了任务结果。人工逐条复核成本过高，模型裁判可以承担规模化初筛，但它需要 rubric、黄金样本和人工抽检约束。在线评测、LLM-as-Judge、人工复核和线上灰度应共同进入质量治理，并把线上问题沉淀成回归资产。

## 关键词

在线评测、LLM-as-Judge、人工抽检、线上灰度、持续优化、裁判偏差

## 学习目标

- 能说明在线评测相对离线 benchmark 的必要性与覆盖差异。
- 能设计 LLM-as-Judge 评分流程，并识别和校正其偏差。
- 能把模型裁判与人工抽检结合，按风险分层采样复核。
- 能把在线评测结果接回灰度发布、回归集和持续优化流程。
