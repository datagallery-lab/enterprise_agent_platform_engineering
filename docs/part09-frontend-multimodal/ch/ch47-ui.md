...
企业 Agent 的前端要展示工具调用状态、等待审批、可编辑证据和任务进度，聊天框只是入口之一。这需要一套把 SSE 事件流翻译成可增量渲染 UI 状态的协议。对话 UI 与流式输出要同时处理消息模型、事件协议、前端状态和可观测性，并把这些设计落到组件、框架选型和前端可观测建设中。

## 关键词

对话 UI、流式输出、SSE、增量渲染、消息模型、前端框架选型

## 学习目标

- 能描述企业 Agent UI 的组成要素，区分它与普通聊天 UI 的差异。
- 能设计 SSE 事件协议，将服务端状态变化映射为前端可增量渲染的消息。
- 能选择合适的前端框架，处理流式输出、工具调用状态与审批等待。
- 能在前端侧建立基本可观测性，帮助定位流式渲染与状态同步问题。
...
可靠交互的关键不是前端多做几个 loading 状态，而是让一次 Agent Run 在前端和后端共享同一条事件链。用户提交问题时，前端生成本地消息占位，同时拿到后端返回的 `run_id`；Runtime 创建 Run 后发出 `message.created`；Planner 决定调用工具时发出 `tool.call.started`；工具执行时持续回写进度和摘要；结果进入消息时，前端 reducer 按 `event_id`、`seq` 和 `run_id` 折叠事件；用户展开证据、复制答案、点踩或重试时，这些 UI 事件也要回写到同一条 trace。
