...
Agent 输出会包含表格、图表、代码、表单和待审批的结构化结论，这些内容需要专门的渲染组件，不能全部退化成 Markdown 文本。Generative UI 与富交互要把可编辑产物、结构化渲染、审批控件、安全边界和前端契约接在 Agent 输出之后。工具调用结果要映射到受控组件，可编辑产物要保留证据链，审批控件要能流转状态，渲染层还要防止 XSS 和越权动作。

## 关键词

Generative UI、可编辑产物、结构化渲染、审批控件、前端契约、XSS 防护

## 学习目标

- 能说明 Generative UI 与普通 Markdown 渲染的差异及其对设计的要求。
- 能设计工具调用结果的结构化渲染组件，展示图表、表格、代码等富内容。
- 能为可编辑产物定义前端契约，支持用户修改并回写 Agent 上下文。
- 能识别 Generative UI 的 XSS 风险并设计安全边界。
