...
文本之外的输入让 Agent 的适用范围大幅扩展，也把新的工程边界前移到入口侧：文件需要异步解析，图片需要权限和脱敏，语音需要延迟与实时控制，这些都不能用“接一个接口”一笔带过。多模态输入与语音 Agent 要把文件、图片、语音、实时控制和上传流水线纳入企业任务界面，并先判断产品边界，再设计文件上传、异步解析、语音链路和实时交互控制。

## 关键词

多模态输入、文件上传、异步解析、语音 Agent、实时交互、多模态权限

## 学习目标

- 能说明文件、图片、语音三类输入在企业任务界面的边界差异。
- 能设计文件上传的异步解析流水线，支持大文件和复杂格式。
- 能描述语音 Agent 的架构，处理延迟、打断与实时反馈控制。
- 能为多模态输入设计权限、脱敏与审计，满足企业合规要求。
